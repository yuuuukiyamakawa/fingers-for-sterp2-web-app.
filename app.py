...
def summarize_english_text(english_text):
    ...
    summarized_english_text = response.choices[0]["message"]["content"].strip()
    return summarized_english_text # 戻り値


# 画像生成と表示の関数
def generate_and_display_image(summarized_english_text):
    ...
    image_prompt = summarized_english_text + "And style is a simple iconic drawing ,lines are bold and thick blacks , background color is white, some accents are neon yellow and light greyish blue"
...
if st.sidebar.button('音声認識と画像生成を開始'):
    ...
    for i in range(total_record_count):
        ...
        # 翻訳文の要約
        state_english_summary = st.empty()
        state_english_summary.write("要約中...") # 箱に案内表示書き込み
        summarized_english_text = summarize_english_text(english_text)  # テキスト要約
        st.write("【要約結果（英語）】")
        state_english_summary.empty()  # 実行中表示の削除
        ...
        # 出力結果を残したまま、次の出力を行う
        st.session_state['output_history'].append((result_text, summarized_text, english_text, summarized_english_text, image))
